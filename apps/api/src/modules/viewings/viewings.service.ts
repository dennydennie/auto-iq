...
export class ViewingsService {
  ...
  async requestViewing(buyerUserId: string, correlationId: string | undefined, listingId: string, body: RequestViewingDto) {
    ...
    if (!buyer) {
      throw new NotFoundException({ code: "RESOURCE_NOT_FOUND", message: "Buyer not found" });
    }
    if (!listing || listing.status !== "PUBLISHED") {
      throw new NotFoundException({ code: "RESOURCE_NOT_FOUND", message: "Listing not found" });
    }
    if (!location) {
      throw new NotFoundException({ code: "RESOURCE_NOT_FOUND", message: "Location not found" });
    }
    ...
    const preferredSlot = parsePreferredSlot(body.preferredDate, body.preferredTime);
    const cover = listing.images?.find((image) => image.isCover) ?? listing.images?.[0] ?? null;
    const viewing = await this.viewingRepository.save(this.viewingRepository.create({
    ...
    await Promise.all([
      this.viewingParticipantRepository.save(this.viewingParticipantRepository.create({
  ...
  async sellerConfirm(sellerUserId: string, correlationId: string | undefined, viewingId: string) {
    const viewing = await this.requireViewing(viewingId);
    if (viewing.sellerUserId !== sellerUserId) {
      throw new NotFoundException({ code: "RESOURCE_NOT_FOUND", message: "Viewing not found" });
    }

    viewing.status = this.viewingStateService.sellerConfirm(viewing.status);
  ...
  async confirm(adminUserId: string, correlationId: string | undefined, viewingId: string, body: ConfirmViewingDto) {
    ...
    if (!admin || !location || !viewing) {
      throw new NotFoundException({ code: "RESOURCE_NOT_FOUND", message: "Viewing or location not found" });
    }
    ...
    await this.auditAdmin(adminUserId, correlationId, "viewing.confirm", viewing.id);
    return this.detail(viewing.id);
  }

  async reschedule(adminUserId: string, correlationId: string | undefined, viewingId: string, body: RescheduleViewingDto) {
    const viewing = await this.requireViewing(viewingId);
    ...
    await this.auditAdmin(adminUserId, correlationId, "viewing.reschedule", viewing.id, body.reason);
    return this.detail(viewing.id);
  }

  async cancel(adminUserId: string, correlationId: string | undefined, viewingId: string, body: CancelViewingDto) {
    const viewing = await this.requireViewing(viewingId);
    ...
    await this.auditAdmin(adminUserId, correlationId, "viewing.cancel", viewing.id, body.reason);
    return this.detail(viewing.id);
  }

  async complete(adminUserId: string, correlationId: string | undefined, viewingId: string, body: CompleteViewingDto) {
    const viewing = await this.requireViewing(viewingId);
    viewing.status = this.viewingStateService.complete(viewing.status, body.outcome);
    viewing.outcomeNote = body.note?.trim() || null;
    viewing.completedAt = new Date();
    await this.viewingRepository.save(viewing);
    await this.upsertAdminParticipant(viewing.id, adminUserId);
    await this.auditAdmin(adminUserId, correlationId, "viewing.complete", viewing.id, body.note);
    return this.detail(viewing.id);
  }
  ...
  private async pageResponse(rows: any[], page: number, limit: number, total: number) {
    return {
  ...
  private async requireViewing(viewingId: string) {
    const viewing = await this.viewingRepository.findByIdWithRelations(viewingId);
    if (!viewing) {
      throw new NotFoundException({ code: "RESOURCE_NOT_FOUND", message: "Viewing not found" });
    }
    return viewing;
  ...
  private async notifyParticipants(viewing: any, template: any, payload: Record<string, unknown>) {
    await this.notificationService.notifyUser({
  ...
  private async auditAdmin(adminUserId: string, correlationId: string | undefined, action: string, viewingId: string, note?: string) {
    await this.auditService.record({
      action,
  ...
  private async upsertAdminParticipant(viewingId: string, adminUserId: string) {
    const participants = await this.viewingParticipantRepository.findByViewingId(viewingId);
    const existing = participants.find((participant) => participant.userId === adminUserId && participant.role === "ADMIN");
    if (existing) {
      existing.confirmed = true;
      await this.viewingParticipantRepository.save(existing);
      return;
    }
    await this.viewingParticipantRepository.save(this.viewingParticipantRepository.create({
      viewingId,
      userId: adminUserId,
      role: "ADMIN",
      confirmed: true,
  ...
  private async markSellerParticipantConfirmed(viewingId: string, sellerUserId: string) {
    const participants = await this.viewingParticipantRepository.findByViewingId(viewingId);
    const seller = participants.find((participant) => participant.userId === sellerUserId && participant.role === "SELLER");
    if (!seller) {
      return;
    }
  ...
  private async toDto(viewing: any) {
    ...
    const participants = viewing.participants ?? await this.viewingParticipantRepository.findByViewingId(viewing.id);
    ...
    return {
      ...
      location: {
        ...
        coordinates: viewing.location.latitude && viewing.location.longitude
          ? { lat: Number(viewing.location.latitude), lng: Number(viewing.location.longitude) }
          : null,
...
function parsePreferredSlot(preferredDate: string, preferredTime: string) {
  ...
  if (!isValidPreferredDate(preferredDate) || Number.isNaN(preferredSlot.getTime())) {
