...
describe("ViewingsService", () => {
  ...
  function viewingFixture(sellerUserId = "seller-1") {
    return {
      ...
      buyer: { id: "buyer-1", fullName: "Buyer One", email: "buyer@example.com", phone: "+2631" },
      seller: { id: sellerUserId, fullName: "Seller One", email: "seller@example.com", phone: "+2632" },
      location: {
        id: "location-1",
        name: "Auto IQ Yard",
      ...
      participants: [
        { userId: "buyer-1", role: "BUYER", confirmed: true, user: { fullName: "Buyer One" } },
        { userId: sellerUserId, role: "SELLER", confirmed: false, user: { fullName: "Seller One" } },
      ],
    };
  }
  ...
  function createService(overrides: Record<number, unknown> = {}) {
    const dependencies = [
      ...
      { save: jest.fn(), create: jest.fn(), findByViewingId: jest.fn() } as never,
      { save: jest.fn(), create: jest.fn(), findByIdWithRelations: jest.fn() } as never,
      new ViewingStateService() as never,
    ];

    Object.assign(dependencies, overrides);
    return new ViewingsService(...(dependencies as unknown as ServiceDependencies));
  }
  ...
  it("rejects a viewing request for an unapproved location", async () => {
    const service = createService({
      ...
      5: { findProfileById: jest.fn().mockResolvedValue({ id: "buyer-1" }), findByRole: jest.fn().mockResolvedValue([]) } as never,
      6: {
        findPublicBySlugOrId: jest.fn().mockResolvedValue({
          id: "listing-1",
          status: "PUBLISHED",
          sellerUserId: "seller-1",
          seller: { id: "seller-1", email: "seller@example.com", phone: "+2631" },
    ...
    await expect(service.requestViewing("buyer-1", "corr-1", "listing-1", {
  ...
  it("rejects invalid preferred slots before persistence", async () => {
    ...
    const service = createService({
      1: { findActiveById: jest.fn().mockResolvedValue({ id: "location-1", name: "Auto IQ Yard" }) } as never,
      5: { findProfileById: jest.fn().mockResolvedValue({ id: "buyer-1" }), findByRole: jest.fn().mockResolvedValue([]) } as never,
      6: {
        findPublicBySlugOrId: jest.fn().mockResolvedValue({
          id: "listing-1",
  ...
  it("lets the listing seller confirm a requested viewing", async () => {
    ...
    const confirmed = await service.sellerConfirm("seller-1", "corr-1", "viewing-1");
  ...
  it("hides viewings from unrelated sellers", async () => {
    const service = createService({
      8: { findByIdWithRelations: jest.fn().mockResolvedValue(viewingFixture("seller-2")) } as never,
    });

    await expect(service.sellerConfirm("seller-1", undefined, "viewing-1"))
      .rejects.toBeInstanceOf(NotFoundException);
  });
});
