...
export class CreateVehicleCatalogue1761300000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const tenant of tenants) await seedTenant(queryRunner, tenant.id);
...
async function createMakeTable(queryRunner: QueryRunner): Promise<void> {
  ...
  await queryRunner.query(
    "CREATE INDEX idx_vehicle_makes_active_catalogue ON vehicle_makes (tenant_id, active, sort_order, name)",
  );
}
...
async function createModelTable(queryRunner: QueryRunner): Promise<void> {
  ...
  await queryRunner.query(
    "CREATE INDEX idx_vehicle_models_active_catalogue ON vehicle_models (tenant_id, make_id, active, sort_order, name)",
  );
}

async function seedTenant(queryRunner: QueryRunner, tenantId: string): Promise<void> {
...
async function setTenantDefault(queryRunner: QueryRunner, table: string): Promise<void> {
  await queryRunner.query(
...
async function enableTenantIsolation(queryRunner: QueryRunner, table: string): Promise<void> {
