    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Tell us how the vehicle will be used and how you plan to buy it.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-5 sm:grid-cols-2">
          <SelectField label="Vehicle purpose" name="vehiclePurpose" value={form.vehiclePurpose} onChange={updateField} options={PURPOSE_OPTIONS} />
          <SelectField label="Payment preference" name="paymentPreference" value={form.paymentPreference} onChange={updateField} options={PAYMENT_OPTIONS} />
          <Field label="Minimum budget (USD)" name="budgetMin" value={form.budgetMin} onChange={updateField} type="number" min="0" step="0.01" />
          <Field label="Maximum budget (USD)" name="budgetMax" value={form.budgetMax} onChange={updateField} type="number" min="0" step="0.01" />
          <Field label="Search radius (km)" name="searchRadiusKm" value={form.searchRadiusKm} onChange={updateField} type="number" min="1" max="1000" step="1" />
          <SelectField label="Delivery preference" name="deliveryPreference" value={form.deliveryPreference} onChange={updateField} options={DELIVERY_OPTIONS} />
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          <CardTitle>Vehicle requirements</CardTitle>
          <CardDescription>Choose the details that matter most. Separate makes or body types with commas.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-5 sm:grid-cols-2">
          ...
          <Field label="Maximum year" name="yearMax" value={form.yearMax} onChange={updateField} type="number" min="1886" max="2200" step="1" />
        </CardContent>
      </Card>
    </>
  );
...
function Field({ label, name, value, onChange, required, placeholder, type = "text", min, max, step }: { label: string; name: TextField; value: string; onChange: (field: TextField, value: string) => void; required?: boolean; placeholder?: string; type?: string; min?: string; max?: string; step?: string }) {
  const id = `account-${String(name)}`;
  return (
    <div className="space-y-2">
      <label className="text-sm font-semibold text-[var(--ink-900)]" htmlFor={id}>{label}{required ? " *" : ""}</label>
      <Input id={id} name={name} value={value} onChange={(event) => onChange(name, event.target.value)} required={required} placeholder={placeholder} type={type} min={min} max={max} step={step} />
    </div>
  );
}

function SelectField({ label, name, value, onChange, options }: { label: string; name: TextField; value: string; onChange: (field: TextField, value: string) => void; options: { value: string; label: string }[] }) {
  const id = `account-${String(name)}`;
  return (
    <div className="space-y-2">
      <label className="text-sm font-semibold text-[var(--ink-900)]" htmlFor={id}>{label}</label>
      <select id={id} name={name} value={value} onChange={(event) => onChange(name, event.target.value)} className="min-h-11 w-full rounded-xl border border-[var(--ink-200)] bg-white px-3.5 text-sm text-[var(--ink-900)] shadow-sm outline-none focus:border-[var(--ink-900)] focus:ring-2 focus:ring-[var(--amber)]/35">
...
function validate(form: FormState) {
  ...
  const radiusError = validateInteger(form.searchRadiusKm, "Search radius", 1, 1000);
  ...
  const mileageError = validateInteger(form.maxMileageKm, "Maximum mileage", 0, 10_000_000);
  if (mileageError) return mileageError;
  const minYearError = validateInteger(form.yearMin, "Minimum year", 1886, 2200);
  if (minYearError) return minYearError;
  const maxYearError = validateInteger(form.yearMax, "Maximum year", 1886, 2200);
  if (maxYearError) return maxYearError;
  const yearMin = integerValue(form.yearMin);
  const yearMax = integerValue(form.yearMax);
  if (yearMin !== null && yearMax !== null && yearMin > yearMax) return "Minimum year cannot be greater than maximum year.";
  return "";
}

function validateInteger(value: string, field: string, min: number, max: number) {
