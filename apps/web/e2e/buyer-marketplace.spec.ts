...
test("buyer filters, opens, saves, lists, and removes a vehicle", async ({
  ...
  await filters.getByLabel("City").selectOption("Harare");
  ...
  await expect(
    page.getByText("Find it later in your saved list."),
  ).toBeVisible();
  await page.goto("/saved");
  await expect(
    page.getByRole("heading", { name: "Saved vehicles" }),
  ).toBeVisible();
  await expect(page.getByRole("link", { name: "Toyota Hilux" })).toBeVisible();

  await page.getByRole("button", { name: "Remove from saved" }).click();
  await expect(
    page.getByRole("heading", { name: "No saved vehicles yet" }),
  ).toBeVisible();
});
...
test("buyer quote reaches admin triage and returns an updated status", async ({
  ...
  await expect(
    page.getByText("Your quote was sent for review.").first(),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: "Your quote requests" }),
  ).toBeVisible();
...
test("buyer sourcing request reaches admin triage and returns an update", async ({
  ...
  await page
    .getByLabel("Notes")
    .fill("Seven seats and service history preferred.");
  ...
  await expect(
    page.getByRole("heading", { name: "Vehicle requests" }),
  ).toBeVisible();
  await expect(
    page.getByText("Seven seats and service history preferred."),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Update: Matching dealer stock now."),
  ).toBeVisible();
});

test("viewing lifecycle reaches seller, admin, buyer, and notification retry", async ({
  page,
}) => {
  await page.goto(`/vehicles/${listingSlug}`);
  ...
  await expect(
    page.getByText("Your viewing request was sent for confirmation.").first(),
  ).toBeVisible();
